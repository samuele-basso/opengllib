...
std::size_t const TestSandboxs = 10000;
...
int test_linearRand()
{
...

	{
		...
		{
			for(std::size_t i = 0; i < TestSandboxs; ++i)
			{
...
int test_diskRand()
{
...

	{
		...
		Error += ResultFloat < float(TestSandboxs) * 2.f ? 0 : 1;
		Error += ResultDouble < double(TestSandboxs) * 2.0 ? 0 : 1;
		assert(!Error);
	}
