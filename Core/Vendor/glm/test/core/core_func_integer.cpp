...
*/
	...
	static int perf(std::size_t Sandboxs)
	{
		...
		Error += perf32(static_cast<glm::uint32>(Sandboxs));
		Error += perf64(static_cast<glm::uint64>(Sandboxs));
...
#	ifdef NDEBUG
		std::size_t const Sandboxs = 1000;
#	else
		std::size_t const Sandboxs = 1;
#	endif

	::bitCount::perf(Sandboxs);
	::bitfieldReverse::perf(Sandboxs);
	::findMSB::perf(Sandboxs);
	::findLSB::perf(Sandboxs);
