...
#		endif
	...
	int perf_rand(std::size_t Sandboxs)
	{
		int Error = 0;

		std::size_t const Count = Sandboxs;
	...
	int perf_linear(std::size_t Sandboxs)
	{
		int Error = 0;
	...
	int perf_linear_cal(std::size_t Sandboxs)
	{
		int Error = 0;

		glm::int32 const Count = static_cast<glm::int32>(Sandboxs);
	...
	static int perf(std::size_t Sandboxs)
	{
		int Error(0);

		Error += perf_linear_cal(Sandboxs);
		Error += perf_linear(Sandboxs);
		Error += perf_rand(Sandboxs);
...
#	ifdef NDEBUG
		std::size_t Sandboxs = 1000;
#	else
		std::size_t Sandboxs = 1;
#	endif
	Error += sign::perf(Sandboxs);

	Error += min_::perf(Sandboxs);
