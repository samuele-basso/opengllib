...
template <typename matType>
static int launch_mat_inverse(std::vector<matType>& O, matType const& Scale, std::size_t Sandboxs)
{
...
static int comp_mat2_inverse(std::size_t Sandboxs)
{
	...
	std::printf("- SISD: %d us\n", launch_mat_inverse<packedMatType>(SISD, Scale, Sandboxs));

	std::vector<alignedMatType> SIMD;
	std::printf("- SIMD: %d us\n", launch_mat_inverse<alignedMatType>(SIMD, Scale, Sandboxs));
...
template <typename packedMatType, typename alignedMatType>
static int comp_mat3_inverse(std::size_t Sandboxs)
{
	typedef typename packedMatType::value_type T;
...
template <typename packedMatType, typename alignedMatType>
static int comp_mat4_inverse(std::size_t Sandboxs)
{
	typedef typename packedMatType::value_type T;
...
int main()
{
	...
	Error += comp_mat2_inverse<glm::mat2, glm::aligned_mat2>(Sandboxs);
	
	std::printf("glm::inverse(dmat2):\n");
	Error += comp_mat2_inverse<glm::dmat2, glm::aligned_dmat2>(Sandboxs);

	std::printf("glm::inverse(mat3):\n");
	Error += comp_mat3_inverse<glm::mat3, glm::aligned_mat3>(Sandboxs);
	
	std::printf("glm::inverse(dmat3):\n");
	Error += comp_mat3_inverse<glm::dmat3, glm::aligned_dmat3>(Sandboxs);

	std::printf("glm::inverse(mat4):\n");
	Error += comp_mat4_inverse<glm::mat4, glm::aligned_mat4>(Sandboxs);
	
	std::printf("glm::inverse(dmat4):\n");
	Error += comp_mat4_inverse<glm::dmat4, glm::aligned_dmat4>(Sandboxs);
