...
template <typename matType, typename vecType>
static int launch_mat_mul_vec(std::vector<vecType>& O, matType const& Transform, vecType const& Scale, std::size_t Sandboxs)
{
	typedef typename matType::value_type T;

	std::vector<vecType> I(Sandboxs);
	O.resize(Sandboxs);

	for(std::size_t i = 0; i < Sandboxs; ++i)
...
static int comp_mat2_mul_vec2(std::size_t Sandboxs)
{
	...
	std::printf("- SISD: %d us\n", launch_mat_mul_vec<packedMatType, packedVecType>(SISD, Transform, Scale, Sandboxs));

	std::vector<alignedVecType> SIMD;
	std::printf("- SIMD: %d us\n", launch_mat_mul_vec<alignedMatType, alignedVecType>(SIMD, Transform, Scale, Sandboxs));
...
template <typename packedMatType, typename packedVecType, typename alignedMatType, typename alignedVecType>
static int comp_mat3_mul_vec3(std::size_t Sandboxs)
{
	typedef typename packedMatType::value_type T;
...
template <typename packedMatType, typename packedVecType, typename alignedMatType, typename alignedVecType>
static int comp_mat4_mul_vec4(std::size_t Sandboxs)
{
	typedef typename packedMatType::value_type T;
...
int main()
{
	std::size_t const Sandboxs = 100000;
	
	int Error = 0;

	std::printf("mat2 * vec2:\n");
	Error += comp_mat2_mul_vec2<glm::mat2, glm::vec2, glm::aligned_mat2, glm::aligned_vec2>(Sandboxs);
	
	std::printf("dmat2 * dvec2:\n");
	Error += comp_mat2_mul_vec2<glm::dmat2, glm::dvec2,glm::aligned_dmat2, glm::aligned_dvec2>(Sandboxs);

	std::printf("mat3 * vec3:\n");
	Error += comp_mat3_mul_vec3<glm::mat3, glm::vec3, glm::aligned_mat3, glm::aligned_vec3>(Sandboxs);
	
	std::printf("dmat3 * dvec3:\n");
	Error += comp_mat3_mul_vec3<glm::dmat3, glm::dvec3, glm::aligned_dmat3, glm::aligned_dvec3>(Sandboxs);

	std::printf("mat4 * vec4:\n");
	Error += comp_mat4_mul_vec4<glm::mat4, glm::vec4, glm::aligned_mat4, glm::aligned_vec4>(Sandboxs);
	
	std::printf("dmat4 * dvec4:\n");
	Error += comp_mat4_mul_vec4<glm::dmat4, glm::dvec4, glm::aligned_dmat4, glm::aligned_dvec4>(Sandboxs);
