...
template <typename matType, typename vecType>
static int launch_vec_mul_mat(std::vector<vecType>& O, matType const& Transform, vecType const& Scale, std::size_t Sandboxs)
{
...
static int comp_vec2_mul_mat2(std::size_t Sandboxs)
{
	...
	std::printf("- SISD: %d us\n", launch_vec_mul_mat<packedMatType, packedVecType>(SISD, Transform, Scale, Sandboxs));

	std::vector<alignedVecType> SIMD;
	std::printf("- SIMD: %d us\n", launch_vec_mul_mat<alignedMatType, alignedVecType>(SIMD, Transform, Scale, Sandboxs));
...
template <typename packedMatType, typename packedVecType, typename alignedMatType, typename alignedVecType>
static int comp_vec3_mul_mat3(std::size_t Sandboxs)
{
	typedef typename packedMatType::value_type T;
...
template <typename packedMatType, typename packedVecType, typename alignedMatType, typename alignedVecType>
static int comp_vec4_mul_mat4(std::size_t Sandboxs)
{
	typedef typename packedMatType::value_type T;
...
int main()
{
	...
	Error += comp_vec2_mul_mat2<glm::mat2, glm::vec2, glm::aligned_mat2, glm::aligned_vec2>(Sandboxs);
	
	std::printf("dvec2 * dmat2:\n");
	Error += comp_vec2_mul_mat2<glm::dmat2, glm::dvec2,glm::aligned_dmat2, glm::aligned_dvec2>(Sandboxs);

	std::printf("vec3 * mat3:\n");
	Error += comp_vec3_mul_mat3<glm::mat3, glm::vec3, glm::aligned_mat3, glm::aligned_vec3>(Sandboxs);
	
	std::printf("dvec3 * dmat3:\n");
	Error += comp_vec3_mul_mat3<glm::dmat3, glm::dvec3, glm::aligned_dmat3, glm::aligned_dvec3>(Sandboxs);

	std::printf("vec4 * mat4:\n");
	Error += comp_vec4_mul_mat4<glm::mat4, glm::vec4, glm::aligned_mat4, glm::aligned_vec4>(Sandboxs);
	
	std::printf("dvec4 * dmat4:\n");
	Error += comp_vec4_mul_mat4<glm::dmat4, glm::dvec4, glm::aligned_dmat4, glm::aligned_dvec4>(Sandboxs);
