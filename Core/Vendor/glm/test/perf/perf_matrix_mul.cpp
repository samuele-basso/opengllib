...
template <typename matType>
static int launch_mat_mul_mat(std::vector<matType>& O, matType const& Transform, matType const& Scale, std::size_t Sandboxs)
{
	typedef typename matType::value_type T;

	std::vector<matType> I(Sandboxs);
	O.resize(Sandboxs);
...
static int comp_mat2_mul_mat2(std::size_t Sandboxs)
{
	...
	std::printf("- SISD: %d us\n", launch_mat_mul_mat<packedMatType>(SISD, Transform, Scale, Sandboxs));

	std::vector<alignedMatType> SIMD;
	std::printf("- SIMD: %d us\n", launch_mat_mul_mat<alignedMatType>(SIMD, Transform, Scale, Sandboxs));
...
template <typename packedMatType, typename alignedMatType>
static int comp_mat3_mul_mat3(std::size_t Sandboxs)
{
	typedef typename packedMatType::value_type T;
...
template <typename packedMatType, typename alignedMatType>
static int comp_mat4_mul_mat4(std::size_t Sandboxs)
{
	typedef typename packedMatType::value_type T;
...
int main()
{
	...
	Error += comp_mat2_mul_mat2<glm::mat2, glm::aligned_mat2>(Sandboxs);
	
	std::printf("dmat2 * dmat2:\n");
	Error += comp_mat2_mul_mat2<glm::dmat2, glm::aligned_dmat2>(Sandboxs);

	std::printf("mat3 * mat3:\n");
	Error += comp_mat3_mul_mat3<glm::mat3, glm::aligned_mat3>(Sandboxs);
	
	std::printf("dmat3 * dmat3:\n");
	Error += comp_mat3_mul_mat3<glm::dmat3, glm::aligned_dmat3>(Sandboxs);

	std::printf("mat4 * mat4:\n");
	Error += comp_mat4_mul_mat4<glm::mat4, glm::aligned_mat4>(Sandboxs);
	
	std::printf("dmat4 * dmat4:\n");
	Error += comp_mat4_mul_mat4<glm::dmat4, glm::aligned_dmat4>(Sandboxs);
