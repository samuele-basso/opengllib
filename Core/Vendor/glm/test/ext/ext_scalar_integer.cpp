...
namespace nextMultiple
{
	...
	int perf_nextMultiple(glm::uint Sandboxs)
	{
		std::vector<glm::uint> Results(Sandboxs * Multiples);

		std::chrono::high_resolution_clock::time_point t0 = std::chrono::high_resolution_clock::now();

		for(glm::uint Source = 0; Source < Sandboxs; ++Source)
	...
	int perf_nextMultipleMod(glm::uint Sandboxs)
	{
		std::vector<glm::uint> Results(Sandboxs * Multiples);
	...
	int perf_nextMultipleNeg(glm::uint Sandboxs)
	{
		std::vector<glm::uint> Results(Sandboxs * Multiples);
	...
	int perf_nextMultipleUFloat(glm::uint Sandboxs)
	{
		std::vector<float> Results(Sandboxs * Multiples);
	...
	int perf_nextMultipleFloat(glm::uint Sandboxs)
	{
		std::vector<float> Results(Sandboxs * Multiples);
	...
	int perf()
	{
		...
		glm::uint const Sandboxs = 10000;

		for(int i = 0; i < 4; ++i)
		{
			std::printf("Run %d :\n", i);
			Error += perf_nextMultiple(Sandboxs);
			Error += perf_nextMultipleMod(Sandboxs);
			Error += perf_nextMultipleNeg(Sandboxs);
			Error += perf_nextMultipleUFloat(Sandboxs);
			Error += perf_nextMultipleFloat(Sandboxs);
			std::printf("\n");
		}
