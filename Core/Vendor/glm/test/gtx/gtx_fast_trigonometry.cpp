...
*/
	...
	int perf_fastCosNew(float Begin, float End, std::size_t Sandboxs)
	{
		std::vector<glm::vec4> Results;
		Results.resize(Sandboxs);

		float const Steps = (End - Begin) / static_cast<float>(Sandboxs);
	...
	int perf_fastCosDeterminisctic(float Begin, float End, std::size_t Sandboxs)
	{
		std::vector<glm::vec4> Results;
		Results.resize(Sandboxs);
...
//		return taylorSeriesRefCos(Angle0_PI);
	...
	int perf_fastCosRef(float Begin, float End, std::size_t Sandboxs)
	{
		std::vector<glm::vec4> Results;
		Results.resize(Sandboxs);
	...
	int perf_fastCosOld(float Begin, float End, std::size_t Sandboxs)
	{
		std::vector<glm::vec4> Results;
		Results.resize(Sandboxs);
	...
	int perf_cos(float Begin, float End, std::size_t Sandboxs)
	{
		std::vector<glm::vec4> Results;
		Results.resize(Sandboxs);
	...
	int perf(std::size_t const Sandboxs)
	{
		int Error = 0;
		...
		Error += perf_cos(Begin, End, Sandboxs);
		Error += perf_fastCosOld(Begin, End, Sandboxs);
		Error += perf_fastCosRef(Begin, End, Sandboxs);
		//Error += perf_fastCosNew(Begin, End, Sandboxs);
		Error += perf_fastCosDeterminisctic(Begin, End, Sandboxs);
...
namespace taylor2
{
	...
	int perf_taylorCosA(float Begin, float End, std::size_t Sandboxs)
	{
	...
	int perf_taylorCosB(float Begin, float End, std::size_t Sandboxs)
	{
		std::vector<float> Results;
		Results.resize(Sandboxs);
	...
	int perf_taylorCosC(float Begin, float End, std::size_t Sandboxs)
	{
		std::vector<float> Results;
		Results.resize(Sandboxs);
	...
	int perf(std::size_t Sandboxs)
	{
		...
		Error += perf_taylorCosA(Begin, End, Sandboxs);
		Error += perf_taylorCosB(Begin, End, Sandboxs);
		Error += perf_taylorCosC(Begin, End, Sandboxs);
